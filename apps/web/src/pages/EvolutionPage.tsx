    ...
    <div className="rounded-lg border border-border bg-[var(--bg-surface)]">
      <div className="px-4 py-3 flex items-start gap-3">
        <div className="text-right shrink-0 min-w-[50px]">
          <p className={`text-lg font-semibold ${r.priorityScore >= 60 ? 'text-red-400' : r.priorityScore >= 30 ? 'text-yellow-400' : 'text-muted'}`}>
            {r.priorityScore}
          </p>
      ...
      {open && (
        <div className="border-t border-border px-4 py-3 space-y-3">
          ...
          {r.evidenceRefs.length > 0 && (
            <div>
              <p className="text-xs text-muted uppercase mb-1">Evidence (real row IDs)</p>
          ...
          {r.status !== 'open' && r.status !== 'in_roadmap' && (
            <p className="text-xs text-muted italic">Status: {r.status}</p>
          )}
        </div>
      )}
    </div>
  ...
  return (
    <div className="flex-1 min-w-0">
      ...
      <div className="space-y-1.5">
        {entries.length === 0 && <p className="text-xs text-muted italic">No tasks in this phase</p>}
        {entries.map((t) => (
          <div key={t.id} className="rounded border border-border bg-[var(--bg-surface)] px-3 py-2">
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono font-medium text-primary">{t.priorityScore}</span>
            ...
            <p className="text-xs text-secondary mt-1 truncate">{t.title}</p>
            <p className="text-xs text-muted mt-0.5">I{t.impact} / R{t.risk}{t.recommendedAgent && ` · ${t.recommendedAgent}`}</p>
          </div>
        ))}
      </div>
    </div>
      ...
      <div className="shrink-0 px-6 pt-5 pb-4 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => qc.invalidateQueries({ queryKey: ['imp-metrics'] })} className="text-muted hover:text-secondary">
          ...
          <div className="max-w-3xl space-y-3">
            <div className="rounded-lg border border-border bg-[var(--bg-surface)] p-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-xs">
                <div><span className="text-muted">Provider efficiency (24h):</span> <span className="font-mono ml-2">{pct(metrics.providerEfficiency)}</span></div>
                <div><span className="text-muted">Patch success rate:</span> <span className="font-mono ml-2">{pct(metrics.patchSuccessRate)}</span></div>
                <div><span className="text-muted">Rollback frequency:</span> <span className="font-mono ml-2">{pct(metrics.rollbackFrequency)}</span></div>
                <div><span className="text-muted">DLQ recovery rate:</span> <span className="font-mono ml-2">{pct(metrics.recoverySuccessRate)}</span></div>
                <div><span className="text-muted">Avg sandbox latency:</span> <span className="font-mono ml-2">{ms(metrics.avgSandboxLatencyMs)}</span></div>
                <div><span className="text-muted">Queue pressure (unreplayed):</span> <span className="font-mono ml-2">{metrics.queuePressure}</span></div>
                <div><span className="text-muted">Build success (7d):</span> <span className="font-mono ml-2">{pct(metrics.recentBuildSuccessRate)}</span></div>
              </div>
