    ...
    <div className="h-full overflow-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-lg font-semibold text-primary">Provider Spend</h1>
            <p className="text-xs text-muted">Cost breakdown by AI provider</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={s.label} className="bg-[var(--bg-surface)] border border-border rounded-lg p-4">
            <div className="flex items-center gap-2 mb-1">
              <s.icon className={`w-4 h-4 ${s.color}`} />
              <span className="text-xs text-muted">{s.label}</span>
            </div>
            <p className="text-xl font-semibold text-primary font-mono">{s.value}</p>
          </div>
        ))}
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border bg-elevated">
              ...
              <th className="text-left px-4 py-2.5 text-xs font-medium text-muted">Share</th>
              <th className="text-left px-4 py-2.5 text-xs font-medium text-muted">Requests</th>
              <th className="text-left px-4 py-2.5 text-xs font-medium text-muted">Last Used</th>
            </tr>
          </thead>
          <tbody>
            {loading ? (
              ...
              data.map((row) => (
                <tr key={row.providerId} className="border-b border-border last:border-0 hover:bg-elevated/30">
                  <td className="px-4 py-3 font-mono text-xs text-primary">{row.providerId}</td>
                  <td className="px-4 py-3 font-mono text-xs text-primary">${row.costUsd.toFixed(6)}</td>
                  ...
                  <td className="px-4 py-3 text-xs text-muted">{row.requests.toLocaleString()}</td>
                  <td className="px-4 py-3 text-xs text-muted">{fmtTs(row.lastUsed)}</td>
                </tr>
              ))
            )}
          </tbody>
