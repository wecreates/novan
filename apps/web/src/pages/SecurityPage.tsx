      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted">· v{s.keyVersion}</span>
          </div>
          <p className="text-xs font-mono text-secondary mt-0.5">{revealed ?? s.valueRedacted}</p>
        ...
        <div className="flex gap-1 shrink-0">
          <button onClick={() => setShowReveal((p) => !p)} title="Reveal (audited)"
            className="px-2 py-1 rounded text-xs text-muted hover:text-yellow-400 hover:bg-yellow-500/10 transition-colors">
            <Eye className="w-3 h-3" />
          </button>
          <button onClick={() => setShowRotate((p) => !p)} title="Rotate"
            className="px-2 py-1 rounded text-xs text-muted hover:text-blue-400 hover:bg-blue-500/10 transition-colors">
      ...
      {showReveal && (
        <div className="mt-3 pt-3 border-t border-border space-y-2">
          ...
          <input value={reason} onChange={(e) => setReason(e.target.value)} placeholder="Why reveal this secret?"
            className="w-full text-xs rounded border border-border bg-bg text-primary px-2 py-1.5 outline-none focus:border-yellow-500/50" />
      ...
      {showRotate && (
        <div className="mt-3 pt-3 border-t border-border space-y-2">
          ...
          <input value={rotateValue} onChange={(e) => setRotateValue(e.target.value)} type="password" placeholder="New value"
            className="w-full text-xs rounded border border-border bg-bg text-primary px-2 py-1.5 outline-none focus:border-blue-500/50" />
          <div className="flex gap-2">
      ...
      <div className="shrink-0 px-6 pt-5 pb-4 border-b border-border">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button onClick={() => qc.invalidateQueries({ queryKey: ['sec-stats'] })} className="text-muted hover:text-secondary">
      ...
      <div className="flex-1 overflow-y-auto px-6 py-4">
        {tab === 'audits' && (
          <div className="space-y-1.5 max-w-5xl">
            {audits.length === 0 && <p className="text-sm text-muted">No security events recorded.</p>}
            {audits.map((a) => (
              <div key={a.id} className="rounded border border-border bg-[var(--bg-surface)] px-3 py-2 flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm text-primary">{a.eventType.replace(/_/g, ' ')}{a.action && ` · ${a.action}`}</p>
                  {a.resource && <p className="text-xs font-mono text-muted truncate">{a.resource}</p>}
                </div>
                {a.userId && <span className="text-xs font-mono text-muted">{a.userId.slice(0, 12)}</span>}
                <span className="text-xs text-muted">{new Date(a.createdAt).toLocaleTimeString()}</span>
              </div>
            ))}
          </div>
        )}
            ...
            <div className="rounded-lg border border-border bg-[var(--bg-surface)] p-4">
              ...
              {integrity ? (
                <div className="grid grid-cols-3 gap-3 text-xs">
                  <div><span className="text-muted">Total:</span> <span className="font-mono ml-2">{integrity.total}</span></div>
                  <div><span className="text-muted">Immutable:</span> <span className="font-mono ml-2 text-green-400">{integrity.immutable}</span></div>
                  <div><span className="text-muted">Mutable:</span> <span className={`font-mono ml-2 ${integrity.mutable > 0 ? 'text-red-400' : 'text-green-400'}`}>{integrity.mutable}</span></div>
                </div>
            ...
            <div className="rounded-lg border border-border bg-[var(--bg-surface)] p-4">
              ...
              {exps.length === 0 ? (
                ...
                <div className="space-y-1">
                  {exps.map((e) => (
                    <div key={e.id} className="flex items-center gap-3 text-xs border-t border-border py-2 first:border-t-0 first:pt-0">
                      ...
                      <span className="font-mono text-secondary">{e.recordCount} records · {e.format}</span>
                      <span className="text-muted">{new Date(e.fromTs).toLocaleDateString()} → {new Date(e.toTs).toLocaleDateString()}</span>
                      {e.downloadRef && <span className="font-mono text-muted ml-auto">{e.downloadRef}</span>}
                    </div>
                  ))}
                </div>
              )}
