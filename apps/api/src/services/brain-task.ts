...
export const OPERATIONS: Record<string, OpSpec> = {
  ...
  'mrr.project': {
    ...
    handler: async (ws) => {
      ...
      return projectMrr(ws)
    },
  },
  'platforms.list_disabled': {
    description: 'R412: Workspace platforms auto-disabled due to 10+ failures + 0 success in 7d.',
    risk: 'low',
    handler: async (ws) => {
      const { listDisabledPlatforms } = await import('./r412-platform-auto-disable.js')
      return { disabled: await listDisabledPlatforms(ws) }
    },
  },
  'platforms.enable': {
    description: 'R412: Operator re-enables a previously auto-disabled platform. Params: platform',
    risk: 'medium',
    handler: async (ws, params) => {
      const p = params as { platform?: string }
      if (!p.platform) return { ok: false, reason: 'platform required' }
      const { enablePlatform } = await import('./r412-platform-auto-disable.js')
      return enablePlatform(ws, p.platform)
    },
  },
  'designs.coverage': {
  ...
  'designs.performance': {
    ...
    handler: async (ws) => {
      ...
      return rankDesignPerformance(ws)
    },
  },
  'queue.stuck': {
  ...
  'business.rollup': {
    ...
    handler: async (ws) => {
      ...
      return businessRollup(ws)
    },
  },
  'business.cron_fanout_test': {
    description: 'R587: Run a no-op fan-out across all businesses (verifies the iteration + skip logic without side effects).',
    risk: 'low',
    handler: async (ws) => {
      const { runForEachBusiness } = await import('./r587-cron-fanout.js')
      return runForEachBusiness(ws, 'fanout_smoke_test', async () => {
        // intentional no-op — exercises lock + heartbeat + iteration
      })
    },
  },
  ...
  'tax.notifications_list': {
    ...
    handler: async (ws, params) => {
      ...
      const yr = p.year ?? new Date().getUTCFullYear()
      const { db } = await import('../db/client.js')
      const { sql } = await import('drizzle-orm')
      try {
        const r = await db.execute(sql`
          SELECT source, bucket, notified_at, ytd_at_notify FROM tax_threshold_notifications
          WHERE workspace_id = ${ws} AND year = ${yr}
          ORDER BY notified_at DESC
        `)
        return { items: (r as unknown as Array<{ source: string; bucket: string; notified_at: number; ytd_at_notify: number }>).map(x => ({
          source: x.source, bucket: x.bucket,
          notifiedAt: Number(x.notified_at),
          ytdAtNotify: Number(x.ytd_at_notify),
        })) }
      } catch { return { items: [] } }
    },
  },
  ...
  for (const step of plan) {
    ...
    try {
      ...
      const INFO_OPS_NO_OUTPUT_GUARD = new Set([
        ...
        'platforms.earnings', 'tax.thresholds', 'tax.notifications_list',
